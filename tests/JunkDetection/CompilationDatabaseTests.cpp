...
TEST(CompilationDatabaseFromFile, parsesCompilationFlagsFromClangMJCommandRaw) {
  auto path = fixtures::junk_detection_compdb_db_produced_from_clang_MJ_raw_json_path();
  const CompilationDatabase database(
      CompilationDatabase::Path(path),
      CompilationDatabase::Flags("-isystem /usr/local/include -isystem /usr/include"));
